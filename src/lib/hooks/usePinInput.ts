...
export function usePinInput({
    ...
    actionRef,
    autoFocus = false,
    defaultValues = Array(6).fill("") as string[],
    type = "numeric",
    otp = false,
    placeholder = "○",
    disabled = false,
    ...
    const fieldRefs = React.useRef<HTMLInputElement[] | null[]>(
    ...
    const clear = React.useCallback(
        ({ focus = false }: PinInputClearOptions = {}) => {
            updateValues(Array(values.length).fill("") as string[]);
    ...
    const fields: PinInputFieldProps[] = values.map(
